...
import morl_baselines.common.parameters as ps #TODO
import random
...
def make_env(env_id, seed, idx, run_name, gamma):
    ...
    """
    print("MO_PPO: Make env new version")
    def thunk():
        render_mode = None
        if idx == 0:
...
class MOPPO(MOPolicy):
    ...
    Modified to support discrete action spaces.
    """
    ...
    def __init__(
        ...
        self.minibatch_size = 64 #int(self.batch_size // num_minibatches) TODO
    ...
    def __collect_samples(self, obs: th.Tensor, done: th.Tensor, weight_support: List[np.ndarray]):
        ...
        for step in range(0, self.steps_per_iteration):
            ...
            for idx in env_term_indices:
                ...
                # Change w for every episode. However store it before changing, in order to return latest used weight vectors when function is retuned.
                old_w = w 
    ...
    def get_value(self, obs, w):
        ...
        return value
    """
    def get_action_and_value(self, obs, w): 
        action_mask = self.get_action_mask(obs)
        with th.no_grad():    
            action, _, _, value = self.networks.get_action_and_value(obs, w, action_mask)
        action = action[0].detach().cpu().numpy().item()
        value =  value[0].detach().cpu().numpy().item()
        return action, value
    """
    ...
    def save_model(self):
        """Save MOPPO model in the same style as Q-net save method.
        
        Args:
            save_replay_buffer: Whether to save the replay buffer (PPO batch)
    ...
    def load_model(self, params):
        """Load MOPPO model in the same style as Q-net load method.
        
        Args:
            path: Path to the saved model file
            load_replay_buffer: Whether to load the replay buffer (PPO batch)
        """        
