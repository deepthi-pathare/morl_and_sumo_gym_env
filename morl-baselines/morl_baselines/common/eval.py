...
def eval_mo(
    ...
    result = {
            "travelled_dist": float(env_un.positions[0, 0] - env_un.init_ego_position),
            "executed_steps": int(env_un.step_),
            "tcop": float(env_un.ego_energy_cost + env_un.ego_driver_cost),
            "energy_cost": float(env_un.ego_energy_cost),
            "driver_cost": float(env_un.ego_driver_cost),
            "avg_speed": float(env_un.ego_speed / env_un.step_)
    }
...
def policy_evaluation_mo(
    ...
    for eval in evals:
        results.append(eval[4])

    avg_results = {
        key: float(np.mean([res[key] for res in results]))
        for key in results[0]
    }
    env_un = env.unwrapped
    failed_episodes = env_un.nb_outside_road + env_un.nb_collisions
    fail_rate = failed_episodes/rep
    succuess_rate = (env_un.nb_max_distance)/rep  
    max_step_rate = (env_un.nb_max_step)/rep 
    summary = {
        "fail_rate": fail_rate,
        "success_rate": succuess_rate,
...
def log_all_multi_policy_metrics(
    ...
    eval_weights: Optional[List[np.ndarray]] = None,
):
    ...
    Args:
        ...
        eval_Weights: evaluation weights used to get current front
    """
    current_front = np.array(current_front)
    if eval_weights is not None:
        eval_weights = np.array(eval_weights)

    pareto_idx = get_non_pareto_dominated_inds(current_front)

    filtered_front = current_front[pareto_idx]
    filtered_weights = eval_weights[pareto_idx] if eval_weights is not None else None
    ...
    if filtered_weights is not None:
        # Add weight columns to the same table
        weight_cols = [f"weight_{i}" for i in range(1, filtered_weights.shape[1] + 1)]
        columns += weight_cols
        data = [f.tolist() + w.tolist() for f, w in zip(filtered_front, filtered_weights)]

    front_table = wandb.Table(columns=columns, data=data)
    wandb.log({"eval/front": front_table})
